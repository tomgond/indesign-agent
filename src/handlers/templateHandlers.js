...
export class TemplateHandlers {
    ...
    static analyze_design_system(args = {}) {
        return response((async () => {
            ...
            const bundle = await this.uxpTool('inspect_document_bundle', {});
            const items = await this.uxpTool('inspect_page_items_v2', { pageIndex, includeHidden: false, includeTextExcerpt: false, includeTextMetadata: false, includeImageMetadata: false, includePathPoints: false, detailLevel: 'summary', limit: args.limit ?? 500 });
...
function inspectionAndChecks(name) {
    const common = `
        ...
        function inspectPageItemsBounded(options){ const warnings = []; const limit = clampInt(options.limit, 200, 1, 500); const offset = clampInt(options.offset, 0, 0, 2147483647); const candidates = getItemCandidates(options, warnings); const matched = []; for (let i=0;i<candidates.length;i++) { const it = candidates[i]; if (!itemMatchesCheapFilters(it, options)) continue; matched.push({ item: it, sourceIndex: i }); } const sliced = matched.slice(offset, offset + limit); const serializer = options.detailLevel === 'deep' ? itemInfoDeep : options.detailLevel === 'standard' ? itemInfoStandard : itemInfoSummary; const items = sliced.map((entry) => serializer(entry.item, entry.sourceIndex, options)); if (offset + items.length < matched.length) warnings.push({ code: 'RESULT_TRUNCATED', message: 'More matching page items are available; use offset/limit to paginate.' }); if (options.detailLevel !== 'deep') warnings.push({ code: 'DEEP_FIELDS_OMITTED', message: 'Deep fields are omitted unless detailLevel=deep and the matching opt-in flags are enabled.' }); if (options.pageIndex != null && options.includeParentItems !== true) warnings.push({ code: 'PARENT_ITEMS_OMITTED', message: 'Parent/master items are omitted by default; set includeParentItems=true to include them.' }); return { items, pagination: { totalMatched: matched.length, returned: items.length, offset, limit, hasMore: offset + items.length < matched.length }, warnings }; }
        function getItemCandidates(options, warnings){ if (options.pageIndex != null) { const page = at(doc.pages, options.pageIndex); if (!page) throw new Error('pageIndex out of range'); if (options.includeParentItems === true) { const items = arr(page.allPageItems || page.pageItems || [], x=>x); if (!page.allPageItems && page.masterPageItems) warnings.push({ code: 'PARENT_ITEMS_DEGRADED', message: 'page.allPageItems was unavailable; falling back to page.pageItems plus masterPageItems.' }); return arr(page.allPageItems || (arr(page.pageItems || [], x=>x).concat(arr(safe(()=>page.masterPageItems, []), x=>x))), x=>x); } warnings.push({ code: 'PARENT_ITEMS_OMITTED', message: 'Parent/master items are omitted by default; set includeParentItems=true to include them.' }); return arr(page.pageItems || page.allPageItems || [], x=>x); } if (options.spreadIndex != null) { const spread = at(doc.spreads, options.spreadIndex); if (!spread) throw new Error('spreadIndex out of range'); return arr(spread.allPageItems || spread.pageItems || [], x=>x); } if (options.allowHeavyInspection !== true) throw new Error('inspect_page_items_v2 requires pageIndex or spreadIndex unless allowHeavyInspection=true'); warnings.push({ code: 'DOCUMENT_WIDE_ITEM_SOURCE', message: 'Document-wide item source used; scope page item inspection with pageIndex or spreadIndex when possible.' }); return arr(doc.allPageItems || doc.pageItems || [], x=>x); }
        ...
        function checkOversetText(options){ const candidates = getItemCandidates(options, []); const matches = []; for (let i=0;i<candidates.length;i++) { const it = candidates[i]; if (!/TextFrame/i.test(itemTypeName(it))) continue; if (!safe(()=>it.overflows, false)) continue; matches.push({ objectId:safe(()=>it.id), objectName:safe(()=>it.name), pageIndex:pageIndexForItem(it), textExcerpt: options && options.includeTextExcerpt === true ? String(safe(()=>it.contents, '') || '').slice(0, 200) : undefined, summary: 'Text frame is overset' }); } const limit = clampInt(options.limit, 200, 1, 500); const offset = clampInt(options.offset, 0, 0, 2147483647); const issues = matches.slice(offset, offset + limit); const warnings = []; if (offset + issues.length < matches.length) warnings.push({ code: 'RESULT_TRUNCATED', message: 'More overset text matches are available; use offset/limit to paginate.' }); return { check:'overset_text', ok: issues.length === 0, issues, warnings }; }
        function checkHiddenOrLockedProblemItems(options){ const candidates = getItemCandidates(options, []); const matches = []; for (let i=0;i<candidates.length;i++) { const it = candidates[i]; const label = readLabel(it) || {}; if ((label.source === 'agent_created' && (safe(()=>it.locked, false) || safe(()=>it.visible, true) === false)) || (label.referenceOnly && safe(()=>it.visible, true) !== false)) { matches.push({ objectId:safe(()=>it.id), objectName:safe(()=>it.name), summary:'Generated/reference item visibility or lock needs review' }); } } const limit = clampInt(options.limit, 200, 1, 500); const offset = clampInt(options.offset, 0, 0, 2147483647); const issues = matches.slice(offset, offset + limit); const warnings = []; if (offset + issues.length < matches.length) warnings.push({ code: 'RESULT_TRUNCATED', message: 'More hidden/locked matches are available; use offset/limit to paginate.' }); return { check:'hidden_or_locked_problem_items', ok: issues.length === 0, issues, warnings }; }
