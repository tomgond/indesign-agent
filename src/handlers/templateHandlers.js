    ...
    static verify_template_roundtrip(args = {}) {
        return response((async () => {
            ...
            const itemsResult = unwrapToolResult(await runPhase('inspect_page_items_v2', () =>
                this.inspectPageItemsRaw({ pageIndex: resolved.pageIndex, includeHidden: true, includeTextExcerpt: args.includeTextExcerpt === true, detailLevel: args.detailLevel || 'summary', trace })
            ));
...
function inspectionAndChecks(name) {
    const common = `
        ...
        function shapeInfoForItem(it,detailLevel){ const type = safe(()=>it.constructor.name, null); const path0 = safe(()=>it.paths && len(it.paths) ? at(it.paths,0) : null, null); const pathPointCount = path0 ? safe(()=>len(path0.pathPoints), 0) : 0; const base = { shapeType:/Oval/i.test(type) ? 'oval' : /Polygon/i.test(type) ? 'polygon' : /GraphicLine/i.test(type) ? 'line' : /Rectangle|TextFrame/i.test(type) ? 'rectangle' : type, cornerRadius:safe(()=>it.topLeftCornerRadius, null), pathPointCount }; if (detailLevel === 'deep' && path0) { base.pathPoints = arr(path0.pathPoints, (pt)=>({ anchor:safe(()=>pt.anchor, null), leftDirection:safe(()=>pt.leftDirection, null), rightDirection:safe(()=>pt.rightDirection, null), pointType:safe(()=>enumString(pt.pointType), null) })); } return base; }
