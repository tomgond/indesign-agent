...
const primitiveToolDefinitions = [
    ...
    { name: 'materialize_inline_svg_asset', description: 'Validate an already-sanitized inline SVG/base64 payload and write it into workspace assets/imports/ for later placement by existing file-based tools. Template mode keeps this file-bound; use create_image_slot or replace_image_in_frame for placement unless the generic/profile-all tool set is active.', inputSchema: { ...schema({ asset: { type: 'object', additionalProperties: true }, svgText: { type: 'string', description: 'Inline SVG text. Provide exactly one of svgText or svgBase64.' }, svgBase64: { type: 'string', description: 'Base64-encoded SVG text. Provide exactly one of svgText or svgBase64.' }, encoding: { type: 'string', enum: ['svgText', 'base64'] }, recommendedFilename: { type: 'string', description: 'Metadata-only hint. Never used for directory control.' }, assetId: { type: 'string', description: 'Stable identifier used for the workspace import subfolder when safe.' }, sha256: { type: 'string', description: 'Expected SHA-256 of the final SVG bytes.' }, byteLength: { type: 'integer', minimum: 1 }, maxSvgBytes: { type: 'integer', minimum: 1, default: 524288 }, metadata: { type: 'object', additionalProperties: true }, safetyReport: { type: 'object', additionalProperties: true }, previewPngBase64: { type: 'string', description: 'Optional preview PNG sidecar.' }, allowEmbeddedImages: { type: 'boolean', default: false } }), anyOf: [{ required: ['asset'] }, { required: ['svgText'] }, { required: ['svgBase64'] }] } },
