...
export const templateToolDefinitions = names.map((name) => ({
    ...
    inputSchema: {
        ...
        properties: {
            ...
            pageIndex: { type: 'number' }, spreadIndex: { type: 'number' }, includeTextExcerpt: { type: 'boolean' }, includeParentItems: { type: 'boolean' }, objectId: { type: 'number' }, objectIds: { type: 'array', items: { type: 'number' } }, groupId: { type: 'number' }, itemId: { type: 'number' }, name: { type: 'string' }, namePrefix: { type: 'string' }, bounds: { type: 'array' }, unit: { type: 'string', enum: ['pt', 'mm'] }, text: { type: 'string' }, width: { type: 'number' }, height: { type: 'number' }, pageWidth: { type: 'number' }, pageHeight: { type: 'number' }, pageSize: { type: 'string', enum: ['A5', 'A3', 'square', 'social_square'] }, orientation: { type: 'string', enum: ['portrait', 'landscape'] }, marginTop: { type: 'number' }, marginBottom: { type: 'number' }, marginLeft: { type: 'number' }, marginRight: { type: 'number' },
